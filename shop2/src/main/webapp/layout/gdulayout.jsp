...
<body class="w3-light-grey">
	...
	<div class="w3-main" style="margin-left: 300px; margin-top: 43px;">
		...
		<div class="w3-row-padding w3-margin-bottom">
			...
			<div class="w3-half">
				<%--최근 7일간 등록 된 게시글 건 수를 막대 그래프와 선 그래프로 나타내기 --%>
				<div class="w3-container w3-padding-16 w3-center">
...
//	   exchangeRate() //수출입은행 환율 정보 조회
	   ...
	   piegraph(2)  // 글쓴이별 게시글 건 수를 파이 그래프로 출력
	   barlinegraph(2) //최근 7일간 게시글 등록 건수를 막대그래프로 출력
   })
...
function piegraph(id) { //2
     $.ajax("${path}/ajax/graph1?id="+id,{
    	 success : function(json) { //json : [{"홍길동":10},{"김삿갓":8},....] 배열 객체로 전달  
...
function pieGraphPrint(arr,id) {
	...
	let datas = []   //글 작성 건수 목록 설정
	$.each(arr,function(index){
		colors[index] = randomColor(0.5) //임의의 색상을 설정
...
function  barlinegraph(id) {
	$.ajax("${path}/ajax/graph2?id="+id,{
		success : function(arr) { //arr :{"2023-06-01":10},{"2023-05-30:20,....."} 날짜의 역순으로 나와야 됨
			let canvas = "<canvas id='canvas2' style='width:100%'></canvas>"
			$("#barcontainer").html(canvas) //새로운 canvas 객체로 생성
			barcontainerPrint(arr,id) //그래프 작성
		},
		error : function(e) {
			alert("서버오류:" +e.status)
...
function barcontainerPrint(arr,id) {
	let colors = [] 
	...
	let config = {
			...
			data : {        //데이터 정보
				datasets : [
				     ...
				     { type: "bar", backgroundColor : colors, laber :'건수', data :datas }
				     ],
			    labels : regdates,
			},
			options : {
			   ...
			   scales : {
				  ...
				  yAxes : [{
					  scaleLabel : { display : true, labelString : "게시물 등록 건 수 "},
					  titck :{beginAtZoro : true}
				  }]
			   }
		   }
	}		
