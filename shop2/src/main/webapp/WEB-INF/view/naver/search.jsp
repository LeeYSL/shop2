...
<script type="text/javascript">
 function naversearch(start) {
	 $.ajax({
	    ...
	    success : function(json) { //json 객체에 전달.네이버에서 전송한 json 객체				
		  //   console.log(json);
	    	 ...
	    	 html += "<table class='w3-table'><tr><td colspan='4' align='center'>"
	    	    + "전체 조회 건 수:" +total+", 현재페이지:" + start +"/" +endpage+"</td></tr>";
	    	    $.each(json.items, function(i,item) {
	    	    	html+="<tr><td>"+num"</td><td>" +item.title 
	    	    	+ "</td><td>" 
	    	    	if($('#type').val() == 'image') {
	    	    		html += "<a href='+ item.link + "'><img srx ='" + item.thumbnail + "'></a><td>"
	    	    		}
	    	    	else {
	    	    		html += "<a href='" + item.link + "'>" + item.linl
	    	    		 + "</a></td><td>" + item.description
	    	    	}
	    	    	html += "</td></tr>";
	    	    	num++
	    	    
			html +="<tr><td colspan='4' align='center'>"; //start : 현재 페이지
			if(start > 1) {
   ...
   /*
        1.페이지의 이동시 num 값을 페이지에 맞추도록 출력
